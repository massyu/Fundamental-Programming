...
void draw() {
  for (int i = 0; i < freq.length; i++) {      // 行(サイコロの目)ごとの繰り返し
    ...
    dx = 30*freq[i];           // 棒の高さ(x方向の長さ)
    if (marginLeft + dx + dH > width) {    // 次が書けないほど右に来ているか? (width はウィンドウの幅)
      fill(0, 0, 0);                  // 色を黒に変更
      noLoop();                       // draw() を呼び出す繰り返しは今回で終わり＝アニメーション終了
    } else {
      fill(360 * i / diceSides, 100, 100);    // 棒ごとに色相を変更
    }
    ellipse(marginLeft+dx, y+marginChar, 12, 12);        // 棒の左上の座標、横、縦
  }
